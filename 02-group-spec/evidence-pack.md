...
> Bản điền dựa trên 01-invidual-workshop/app-teardown.md.
> Trạng thái xác minh: ✅ = đã xem tận mắt / fetch được nguyên văn (E1, E2, E3, E7). Các mục E4–E6 là **mô tả theo nguồn**, cần mở link chụp xác nhận khi nộp (xem `evidence-screenshots-guide.md`).

## 1. Nhóm và track

**Tên nhóm:** Nguyễn Trọng Khánh — 2A202600796
...
|---|---|---|---|
| Form booking chỉ hỏi điểm đến/ngày/phòng/khách, không hỏi mục đích chuyến đi, ngân sách, nhóm đi cùng, sở thích | `01-invidual-workshop/image.png` | Low-confidence | Hệ thống giả định user đã biết rõ mình muốn gì; user mơ hồ không được dẫn dắt |
| Nhiều ưu đãi/combo/voucher hiển thị cùng lúc, mỗi gói có điều kiện & quyền lợi riêng | `image-1.png`, `image-2.png` | Low-confidence | Decision fatigue — user phải tự đọc & tự so sánh nhiều gói |
| Khó nhận ra điểm khác biệt dẫn đến chênh lệch giá giữa các gói/phòng | `image-3.png` | Low-confidence / Failure | User không hiểu vì sao gói A đắt hơn gói B → mất niềm tin, phân vân |
| Đã thử scenario "gia đình/couple đi Phú Quốc / Nha Trang" trong luồng tìm kiếm | `image-4.png` | Low-confidence | Không có flow hỏi lại tiêu chí; user tự gánh việc lọc |
...
### Screenshot evidence (chụp theo `evidence-screenshots-guide.md`)

> Bỏ ảnh vào `02-group-spec/evidence/` đúng tên là các ảnh dưới tự hiện. Chưa có ảnh thì link sẽ gãy — chụp xong sẽ ok.
